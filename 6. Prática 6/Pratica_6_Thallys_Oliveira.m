...
v_vetor = [zeros(size(t2)), v, zeros(size(t2)), flip(v)];
a_vetor = [zeros(size(t3)), a, zeros(size(t3)), flip(a)];
j_vetor = [zeros(size(t4)), j, zeros(size(t4)), flip(j)];

% Redimensionando os vetores para o plot
t = 0:6.001/4001:6.001;
theta = pi/3 * t;
